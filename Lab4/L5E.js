...
function render()
{
   ...
   var middleSphere = mult(mv, mult(translate(0, 0, 0), rotateY(ry)));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(middleSphere));
   ...
   //Lamp post
   var lampTF = mult(mv, translate(-2.45, 1, 0));
   lampTF=mult(lampTF, scalem(.25,4.0,.25))
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(lampTF));
   setMaterial(material.clay);
  urgl.drawSolidCube(1);


   //Light at top of lamp post
   var lightBulbTF = mult(mv, translate(-2.5, 3, 0));
   lightBulbTF =mult(lightBulbTF, scalem(rez,rez,rez));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(lightBulbTF));
   gl.uniform1f(lighting, 0);
   urgl.drawSolidSphere(0.5, rez, rez);
   for( var i=0; i<index; i+=3)
        gl.drawArrays(gl.TRIANGLES, i, 3);
   ...
   floatingLightTF = mult(floatingLightTF,scalem(.1,.1,.1));
   gl.uniformMatrix4fv(mvLoc, gl.FALSE, flatten(floatingLightTF));
   gl.uniform1f(lighting, 0);
   for( var i=0; i<index; i+=3)
      gl.drawArrays(gl.TRIANGLES, i, 3);
   gl.uniform1f(lighting, 1);
