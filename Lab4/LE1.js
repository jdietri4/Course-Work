...
var shapes = {
	...
	octahedron: { points: [], normals: [], start: 0, size: 0, type: 0 } //Jacob Dietrich
...
var octahedronVerts = [ //Jacob Dietrich
    vec4(-1, 0, 0, 1), //0
    vec4( 1, 0, 0, 1), //1
    vec4( 0,-1, 0, 1), //2
    vec4( 0, 1, 0, 1), //3
...
var octahedronLookups = [ //Jacob Dietrich
    0, 1, 4,   1, 2, 4,
    2, 3, 4,   3, 0, 4,
    0, 1, 5,   1, 2, 5,
    2, 3, 5,   3, 0, 5,
...
//Reset faceNum to 0 (Jacob Dietrich)
faceNum = 0;

//create a normal for each face of the octahedron (Jacob Dietrich)
for (var i = 0; i < octahedronLookups.length; i++) {
	shapes.octahedron.points.push(octahedronVerts[octahedronLookups[i]]);
	shapes.octahedron.normals.push(normalsList[faceNum]);
	if (i % 6 == 5) faceNum++;
}
...
//    to (start + num)
//----------------------------------------------------------------------------
//Added, but not written by Jacob Dietrich
...
window.onload = function init() {
	...
	loadShape(shapes.octahedron, gl.TRIANGLES); //Added by Jacob Dietrich
...
function render() {
	...
	gl.drawArrays(shapes.octahedron.type, shapes.octahedron.start, shapes.octahedron.size); //Added by Jacob Dietrich
